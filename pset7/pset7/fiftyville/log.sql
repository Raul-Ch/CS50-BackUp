...
SELECT passport_number FROM passengers WHERE flight_id IN
(SELECT destination_airport_id FROM flights WHERE origin_airport_id IN
(SELECT id FROM airports WHERE city = "Fiftyville") AND day = 29 AND month = 7 AND year = 2021 ORDER BY month, day, hour, minute LIMIT 1) INNER JOIN people ON passengers.passport_number = people.passport_number;