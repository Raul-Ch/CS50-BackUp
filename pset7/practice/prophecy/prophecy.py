...
with open("students.csv", "r") as file:
    ...
    for assignment in assignments:
        student_id = db.execute("SELECT id FROM students WHERE student_name = ?", assignment["Student"]).fetchone()['id']
        house_id = db.execute("SELECT id FROM houses WHERE house = ?", assignment["House"]).fetchone()['id']
